...
@section('title', 'Share Sale List')
@section('admin-content')
<main>
    <div class="container">
        <div class="heading-title p-2 my-2">
            <span class="my-3 heading "><i class="fas fa-home"></i> <a class="" href="{{route('admin.index')}}">Home</a> >Share Sale  List</span>
        </div>
        <div class="card">
            <div class="card-header d-flex">
                <div class="table-head text-left"><i class="fas fa-table me-1"></i>Share Sale List</div>
                                   ...
                                   <td>{{$order->total_amount}}</td>
                                    <td>
                                        @if(Auth::user()->action_process == 1)
                                            @if ($order->status == 'Share Sale')
                                            <a href="{{route('offer.delivery',$order->id)}}" onclick="way({{$order->id}})" class="btn btn-edit procesing" data-bs-toggle="modal" data-bs-target="#myModal">Share Sale</a>
                                            @endif
                                        @endif
                                    </td>
