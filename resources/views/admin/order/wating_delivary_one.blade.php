...
@section('title', 'Waiting Normal order Devlivery')
    ...
    <div class="container">
        ...
        <div class="card">
            <div class="card-header d-flex">
                <div class="table-head text-left"><i class="fas fa-table me-1"></i>Waiting Normal order Devlivery List </div>
                                   ...
                                   <td>{{$order->total_amount}}</td>
                                    <td>
                                        @if(Auth::user()->action_process == 1)
                                            @if ($order->status == 'wd_1')
                                            <a href="{{route('normal.delivered',$order->id)}}" onclick="return confirm('are you sure! Order now confirm')" class="btn btn-edit">Waiting Delivery</a>
                                            @endif
                                        @endif
                                    </td>
